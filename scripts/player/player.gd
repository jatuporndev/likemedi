...
func _physics_process(delta: float) -> void:
	...
	if _attack_flash_time > 0.0:
		_attack_flash_time -= delta
		attack_marker.visible = _attack_flash_time > 0.0
...
func _update_animation(delta: float) -> void:
	...
	_set_animation("run" if is_running else "idle")
...
func _set_animation(animation_name: String) -> void:
	...
	_animation_time = 0.0
	if animation_name == "run" and run_texture != null:
...
func _build_chat_bubble() -> void:
	...
	_chat_bubble_box.offset_left = -68.0
	_chat_bubble_box.offset_top = -140.0
	_chat_bubble_box.offset_right = 68.0
	_chat_bubble_box.offset_bottom = -106.0
	...
	chat_margin.add_theme_constant_override("margin_top", 4)
	chat_margin.add_theme_constant_override("margin_bottom", 4)
...
func _on_chat_message_received(sender_id: int, message: String) -> void:
	...
	_chat_bubble_time = CHAT_BUBBLE_SECONDS
